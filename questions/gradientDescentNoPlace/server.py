...
def generate(data):
    ...
    def update_rule(theta0, theta1, learning_rate, coeff1, exp, coeff2) -> float:
        ...
        next_theta0 = theta0 - learning_rate * (
            coeff1 + exp * theta0 ** (exp - 1) * theta1
        )
        next_theta1 = theta1 - learning_rate * (theta0**exp + coeff2)
        return next_theta0, next_theta1

    next_theta0, next_theta1 = update_rule(
        theta0, theta1, learning_rate, coeff1, exp, coeff2
    )
    data["correct_answers"]["number1"] = next_theta0
    data["correct_answers"]["number2"] = next_theta1

    next_theta0, _ = update_rule(
        next_theta0, next_theta1, learning_rate, coeff1, exp, coeff2
    )
